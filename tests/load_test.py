...
def hit(_index):
    ...
    with urlopen(f"{TARGET}/health", timeout=10) as response:
...
if __name__ == "__main__":
    ...
    report = {"target": TARGET, "requests": REQUESTS, "concurrency": CONCURRENCY, "failures": failures, "average_ms": round(statistics.mean(latencies), 2), "p95_ms": round(p95, 2), "accepted": failures == 0 and p95 <= P95_LIMIT_MS}
